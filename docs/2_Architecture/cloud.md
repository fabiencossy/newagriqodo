# CLOUD.MD — PRINCIPES ARCHITECTURAUX AGRICODO V2
...
**Règle inviolable** : Aucune modification côté Odoo. Agricodo parle uniquement via:
- **REST API Odoo** (lectures/écritures via `xmlrpc` ou `/api/`)
- **Webhooks Odoo** (Odoo → Agricodo, événements push)

**Implication:** Toute nouvelle donnée Odoo doit être mappée, validée et stockée localement en Agricodo.
